...
/**
 * @file    Serial_Test.c
 * @brief   串口数据包接收测试（参考示例）
 * @details 接收上位机发来的 6 字节坐标数据包（0xAA XH XL YH YL 0xFF），
 *          在 OLED 上显示 X、Y 坐标
 *
 * 数据包格式（6 字节）：
 ...
 * 结果现象：
 *   - 上电后 OLED 清空
 *   - 串口调试助手发送 "AA 00 32 00 64 FF" → OLED 显示 "X:50 Y:100"
 *   - 串口调试助手发送 "AA 01 2C 02 58 FF" → OLED 显示 "X:300 Y:600"
 *
