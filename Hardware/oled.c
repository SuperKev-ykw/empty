...
void OLED_WR_Byte(uint8_t dat, uint8_t mode)
{
    ...
    // 3. 启动传输（不等待完成，让硬件在后台发送）
    DL_I2C_startControllerTransfer(OLED_INST, 0x3C, DL_I2C_CONTROLLER_DIRECTION_TX, 2);
}

// 批量写数据（硬件 I2C 逐个发送，不等待完成）
static void OLED_WriteData_Bulk(const uint8_t *data, uint8_t len)
{
    uint8_t i;
    for (i = 0; i < len; i++) {
        uint8_t txData[2];
...
void OLED_ShowSignedNum(u8 x,u8 y,s32 num,u8 len,u8 size1)
{
	...
	for (i = 0; i < len; i++)
	{
		OLED_ShowChar(x + (i + 1) * (size1 / 2), y,
		              num_abs / OLED_Pow(10, len - i - 1) % 10 + '0', size1);
	}
}
...
void OLED_ShowHexNum(u8 x,u8 y,u32 num,u8 len,u8 size1)
{
	...
	for (i = 0; i < len; i++)
	{
		...
		if (single_num < 10)
			OLED_ShowChar(x + i * (size1 / 2), y, single_num + '0', size1);
		else
			OLED_ShowChar(x + i * (size1 / 2), y, single_num - 10 + 'A', size1);
	}
}
...
void OLED_ShowBinNum(u8 x,u8 y,u32 num,u8 len,u8 size1)
{
	...
	for (i = 0; i < len; i++)
	{
		OLED_ShowChar(x + i * (size1 / 2), y,
		              num / OLED_Pow(2, len - i - 1) % 2 + '0', size1);
	}
}
...
void OLED_ShowFloatNum(u8 x,u8 y,double num,u8 int_len,u8 fra_len,u8 size1)
{
	...
	int_num = (u32)num;
	num -= int_num;
	pow_num = OLED_Pow(10, fra_len);
	fra_num = (u32)(num * pow_num + 0.5);  // 四舍五入
	int_num += fra_num / pow_num;
	
	OLED_ShowNum(x + (size1 / 2), y, int_num, int_len, size1);
	OLED_ShowChar(x + (int_len + 1) * (size1 / 2), y, '.', size1);
	OLED_ShowNum(x + (int_len + 2) * (size1 / 2), y, fra_num, fra_len, size1);
}
...
void OLED_Printf(u8 x,u8 y,u8 size1,char *format,...)
{
	...
	va_end(arg);
	OLED_ShowString(x, y, (u8 *)string, size1);
}
