 ...
 * @param num 要显示的数字
 * @param len 数字的显示位数
 * @param size1 字符大小（如12、16、24）
 */
void OLED_ShowSignedNum(u8 x,u8 y,s32 num,u8 len,u8 size1);
 ...
 * @param size1 字符大小
 */
void OLED_ShowHexNum(u8 x,u8 y,u32 num,u8 len,u8 size1);
 ...
 * @param size1 字符大小
 */
void OLED_ShowBinNum(u8 x,u8 y,u32 num,u8 len,u8 size1);
 ...
 * @param size1 字符大小
 */
void OLED_ShowFloatNum(u8 x,u8 y,double num,u8 int_len,u8 fra_len,u8 size1);

/**
 * @brief 格式化打印显示（类似标准库printf函数）
 *
 * 将格式化字符串和可变参数组合后显示在OLED屏幕上，
 * 支持所有标准printf格式说明符，可混合显示字符串、整数、浮点数等。
 *
 * @param x 字符串左上角的起始横坐标（像素单位）
 *          - 范围：0 ~ 127（屏幕宽度128像素）
 *          - 超出屏幕范围的内容将不显示
 *          - 例如 x=0 表示从屏幕最左侧开始
 *
 * @param y 字符串左上角的起始纵坐标（像素单位）
 *          - 范围：0 ~ 63（屏幕高度64像素）
 *          - 超出屏幕范围的内容将不显示
 *          - 例如 y=0 表示从屏幕最顶部开始
 *          - 例如 y=16 表示从第17行像素开始（适合第二行显示）
 *
 * @param size1 字符/字体大小（像素宽度）
 *              - 常用取值：
 *                - 16：8x16点阵字体（宽8像素，高16像素）
 *                - 12：6x8点阵字体（宽6像素，高8像素）
 *              - 字体大小会影响字符间距和每行可显示的字符数量
 *              - 使用16号字体时，每行最多显示 128/8 = 16 个字符
 *              - 使用12号字体时，每行最多显示 128/6 ≈ 21 个字符
 *
 * @param format 格式化字符串（C风格字符串）
 *               - 支持标准printf格式说明符：
 *                 - %d 或 %i：有符号十进制整数（int）
 *                 - %u：无符号十进制整数（unsigned int）
 *                 - %x：无符号十六进制整数（小写，如 0xab）
 *                 - %X：无符号十六进制整数（大写，如 0xAB）
 *                 - %o：无符号八进制整数
 *                 - %f：浮点数（如 3.14159）
 *                 - %.2f：保留2位小数的浮点数（如 3.14）
 *                 - %c：单个字符
 *                 - %s：字符串
 *                 - %%：显示百分号本身
 *               - 字符串中可混合普通文本和格式说明符
 ...
 *
 * @param ... 可变参数列表
 *            - 参数数量和类型必须与format中的格式说明符一一对应
 *            - 参数按从左到右的顺序依次替换format中的说明符
 *            - 例如 format="Key:%d Val:%d"，则需要传入两个int类型参数
 *
 * @note 使用示例：
 *       OLED_Printf(0, 0, 16, "Hello World");                    // 纯文本
 *       OLED_Printf(0, 0, 16, "Key:%d", key_num);                // 显示整数
 *       OLED_Printf(0, 16, 16, "Temp:%.1fC", 25.6);              // 显示浮点数
 *       OLED_Printf(0, 32, 16, "Hex:0x%X", 0xAB);                // 显示十六进制
 *       OLED_Printf(0, 48, 16, "%s:%d", "ADC", adc_val);         // 混合字符串和整数
 *       OLED_Printf(0, 0, 16, "Bat:%d%%", 85);                   // 显示百分号
 *
 * @note 注意事项：
 *       - 内部缓冲区大小为128字节，格式化后的字符串总长度不应超过127个字符
 *       - 调用此函数后内容会直接写入OLED显存并刷新到屏幕
 *       - 不支持中文字符显示（需要额外实现中文字库）
 *       - 浮点数显示精度受限于C标准库的vsprintf实现
 */
void OLED_Printf(u8 x,u8 y,u8 size1,char *format,...);
